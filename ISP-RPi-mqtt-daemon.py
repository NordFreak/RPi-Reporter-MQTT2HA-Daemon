...
def getFanSpeed():
    ...
    try:
        ...
        if fan_path:
            # Lese den Inhalt der ersten gefundenen Datei
            with open(fan_path[0], 'r') as f:
                content = f.read().strip()
                # Konvertiere den gelesenen Wert in eine ganze Zahl
                rpi_fan_speed = int(content)

    except Exception as e:
        # Bei einem Fehler (z.B. Datei nicht gefunden, Lesefehler) wird fan_speed 0 gesetzt
        print_line(f'! Failed to read RPi fan speed: {e}', error=True)
        rpi_fan_speed = 0
